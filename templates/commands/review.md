...
now run during /rapidspec.proposal (Design Review) and /rapidspec.apply (Architecture Validation)
...
```
User: "/rapidspec.review add-loading-state"
...
Ready to commit: /rapidspec.commit add-loading-state
```

### Example 2: Warnings Found (DB + Component Changes)

```
User: "/rapidspec.review prevent-duplicate-smartlinks"
...
```
User: "/rapidspec.review add-admin-panel"
...
**Standard flow:**
```
/rapidspec.apply add-feature
→ /rapidspec.review add-feature
→ Fix warnings (if any)
→ /rapidspec.commit add-feature
```

**Quick iteration:**
```
/rapidspec.apply add-feature
→ /rapidspec.commit add-feature  (skip review)
```

**Mid-implementation check:**
```
/rapidspec.apply add-feature (Tasks 1-2)
