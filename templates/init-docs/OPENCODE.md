...
- New feature? → `/rapidspec.proposal [name]`
- Ready to implement? → `/rapidspec.apply [change-id]`
- Quality check? → `/rapidspec.review [change-id]`
- Done? → `/rapidspec.commit [change-id]`
...
*Generated for opencode on [TIMESTAMP] by RapidSpec Init*
