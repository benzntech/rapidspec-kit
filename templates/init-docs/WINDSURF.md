# Windsurf - RapidSpec Quick Start

This project was initialized with **Windsurf** on [TIMESTAMP].

## RapidSpec Commands (10 Total)

### Core Workflow
1. `/rapidspec.constitution` - Principles | **First**
2. `/rapidspec.proposal` - Plan feature | **Before impl**
3. `/rapidspec.apply` - Implement | **After proposal**
4. `/rapidspec.review` - Quality check | **Before commit**
5. `/rapidspec.commit` - Save & document | **After apply**

### Advanced
6. `/rapidspec.archive` - Mark complete
7. `/rapidspec.triage` - Categorize issues
8. `/rapidspec.finalize` - Release prep
9. `/rapidspec.resolve-parallel` - Merge features
10. `/rapidspec.umb` - Update memory

---

## Workflow

✅ Constitution → Proposal → Apply → Review → Commit → Archive

❌ Never: Skip proposal | Old commands | Ignore review | Skip memory

---

## Memory Bank (`.rapidspec/memory/`)

- `constitution.md` - Governance
- `productContext.md` - Architecture
- `activeContext.md` - Current work
- `systemPatterns.md` - Patterns
- `decisionLog.md` - Decisions
- `progress.md` - Work history

---

## Quick Commands

- Plan feature: `/rapidspec.proposal [name]`
- Implement: `/rapidspec.apply [change-id]`
- Check quality: `/rapidspec.review [change-id]`
- Save work: `/rapidspec.commit [change-id]`

---

## Learn More

- Workflow: `AGENTS.md`
- Context: `.rapidspec/memory/productContext.md`
- Windsurf: [windsurf.com](https://windsurf.com)
- RapidSpec: [github.com/benzntech/rapidspec-kit](https://github.com/benzntech/rapidspec-kit)

---

*Generated for Windsurf on [TIMESTAMP] by RapidSpec Init*
