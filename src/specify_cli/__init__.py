...
TAGLINE = "RapidSpec-Kit - Spec-Driven Development Toolkit"
class StepTracker:
...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    repo_owner = "benzntech"
    repo_name = "rapidspec-kit"
    if client is None:
        client = httpx.Client(verify=ssl_context)
    ...
    # Pattern matches: rapidspec-template-{assistant}-{script_type}-v*.zip
    pattern = f"rapidspec-template-{ai_assistant}-{script_type}-"
    matching_assets = [
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .rapidspec/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".rapidspec" / "scripts"
...
def create_model_init_guide(project_path: Path, ai_assistant: str, tracker: StepTracker | None = None) -> bool:
    ...
    # Try to find template in the extracted project first (most reliable)
    template_path = project_path / ".rapidspec" / "templates" / "init-docs" / template_filename
    
    if not template_path.exists():
        # Try package installation location as fallback
        template_path = Path(__file__).parent.parent.parent / "templates" / "init-docs" / template_filename
