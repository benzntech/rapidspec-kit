...
rewrite_paths() {
  sed -E \
    -e 's@(/?)memory/@.rapidspec/memory/@g' \
    -e 's@(/?)scripts/@.rapidspec/scripts/@g' \
    -e 's@(/?)templates/@.rapidspec/templates/@g'
}
...
build_variant() {
  ...
  SPEC_DIR="$base_dir/.rapidspec"
  mkdir -p "$SPEC_DIR"

  [[ -d memory ]] && { cp -r memory "$SPEC_DIR/"; echo "Copied memory -> .rapidspec"; }
  ...
  if [[ -d scripts ]]; then
    ...
    case $script in
      sh)
        [[ -d scripts/bash ]] && { cp -r scripts/bash "$SPEC_DIR/scripts/"; echo "Copied scripts/bash -> .rapidspec/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
      ps)
        [[ -d scripts/powershell ]] && { cp -r scripts/powershell "$SPEC_DIR/scripts/"; echo "Copied scripts/powershell -> .rapidspec/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
    esac
  ...
  ( cd "$base_dir" && zip -r "../rapidspec-template-${agent}-${script}-${NEW_VERSION}.zip" . )
  echo "Created $GENRELEASES_DIR/rapidspec-template-${agent}-${script}-${NEW_VERSION}.zip"
}
...
ls -1 "$GENRELEASES_DIR"/rapidspec-template-*-"${NEW_VERSION}".zip
