...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $rapidspecFile = Join-Path $OutputDir "rapidspec.$name.$Extension"

        switch ($Extension) {
            'toml' {
                $bodyEscaped = $body -replace '\\', '\\'
                $output = "description = `"$description`"`n`nprompt = `"`"`"`n$bodyEscaped`n`"`"`""
                Set-Content -Path $rapidspecFile -Value $output -NoNewline

                # Generate deprecated speckit.* commands with warning
                $deprecatedBody = "⚠️ DEPRECATED: Use /rapidspec.$name instead`n`n$body"
                $bodyEscaped = $deprecatedBody -replace '\\', '\\'
                $output = "description = `"$description (DEPRECATED)`"`n`nprompt = `"`"`"`n$bodyEscaped`n`"`"`""
                $speckitFile = Join-Path $OutputDir "speckit.$name.$Extension"
                Set-Content -Path $speckitFile -Value $output -NoNewline
            }
            'md' {
                Set-Content -Path $rapidspecFile -Value $body -NoNewline

                # Generate deprecated speckit.* commands with warning
                $deprecatedBody = "⚠️ **DEPRECATED**: Use ``/rapidspec.$name`` instead`n`n$body"
                $speckitFile = Join-Path $OutputDir "speckit.$name.$Extension"
                Set-Content -Path $speckitFile -Value $deprecatedBody -NoNewline
            }
            'agent.md' {
                Set-Content -Path $rapidspecFile -Value $body -NoNewline
...
function Generate-CopilotPrompts {
    ...
    # Generate prompts for both rapidspec and deprecated speckit agent files
    $agentFiles = Get-ChildItem -Path "$AgentsDir/rapidspec.*.agent.md" -File -ErrorAction SilentlyContinue
